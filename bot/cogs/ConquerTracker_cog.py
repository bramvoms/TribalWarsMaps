...
class ConquerTracker(commands.Cog):
    ...
    async def create_tables(self):
        ...
        # Onthoud per wereld tot waar we al verwerkt hebben
        await self.bot.db.execute("""
    ...
    async def toggle_tracking(self, guild_id: int, channel_id: int, world: str, tribe_tag: str):
        ...
        if exists:
            ...
            any_left = await self.bot.db.fetchval("SELECT 1 FROM conquer_settings_v2 LIMIT 1;")
        ...
        # Zet world state als die nog niet bestaat
        await self.bot.db.execute("""
    ...
    async def _get_since_for_world(self, world: str) -> int:
        ...
        # Fallback: gebruik de oudste starting_unix_timestamp van alle settings voor deze wereld
        min_start = await self.bot.db.fetchval("""
            SELECT MIN(starting_unix_timestamp)
            FROM conquer_settings_v2
            WHERE world = $1;
        """, world)

        since = int(min_start) if min_start else int(datetime.utcnow().timestamp())
        await self.bot.db.execute("""
            INSERT INTO conquer_world_state_v2 (world, last_since)
            VALUES ($1, $2)
    ...
    async def check_conquers(self):
        ...
        for world in worlds:
            try:
                ...
                # Haal alle tracking channels voor deze wereld 1x op
                ...
                # Zet since door naar net na de hoogste timestamp die we zagen
                # plus 1 om herhaling te vermijden bij exact gelijke timestamp
