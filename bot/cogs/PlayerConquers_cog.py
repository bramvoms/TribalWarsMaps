...
class PlayerConquers(commands.Cog):
    ...
    async def create_tables(self):
        await self.db.execute("""
            CREATE TABLE IF NOT EXISTS player_conquer_settings_v2 (
                guild_id BIGINT NOT NULL,
                channel_id BIGINT NOT NULL,
                world TEXT NOT NULL,
                player_id BIGINT NOT NULL,
                starting_unix_timestamp BIGINT NOT NULL,
                PRIMARY KEY (guild_id, channel_id, world, player_id)
            );
        """)

        await self.db.execute("""
            CREATE TABLE IF NOT EXISTS player_conquer_data_v2 (
                world TEXT NOT NULL,
        ...
        await self.db.execute("""
            CREATE TABLE IF NOT EXISTS player_conquer_messages_v2 (
                guild_id BIGINT NOT NULL,
                channel_id BIGINT NOT NULL,
                world TEXT NOT NULL,
                village_id BIGINT NOT NULL,
        ...
        await self.db.execute("""
            CREATE TABLE IF NOT EXISTS player_conquer_world_state_v2 (
    ...
    async def cog_load(self):
        ...
        logger.info("[PlayerConquers] Loaded")

    async def cog_unload(self):
        if self.player_check_conquers.is_running():
            self.player_check_conquers.cancel()

        if self.session and not self.session.closed:
            await self.session.close()

        logger.info("[PlayerConquers] Unloaded")
    ...
    async def on_ready(self):
        ...
        rows = await self.db.fetch("SELECT 1 FROM player_conquer_settings_v2 LIMIT 1;")
        if rows and not self.player_check_conquers.is_running():
            self.player_check_conquers.start()
            logger.info("[PlayerConquers] Background player_check_conquers loop started via on_ready.")

        self.loop_initialized = True

    async def get_player_id(self, world: str, player_name: str):
    ...
    @commands.command(name="playerconquertracker")
    @commands.is_owner()
    async def playerconquertracker(self, ctx: commands.Context, world: str, *, player_name: str):
        world = world.strip().lower()
        player_name = player_name.strip()
        await self.toggle_tracking(ctx.guild.id, ctx.channel.id, world, player_name)

    async def toggle_tracking(self, guild_id: int, channel_id: int, world: str, player_name: str):
        player_data = await self.get_player_id(world, player_name)
        channel = self.bot.get_channel(channel_id)

        if not player_data:
            if channel:
                await channel.send(
                    f"Speler `{player_name}` niet gevonden op wereld `{world}`. "
                    f"Gebruik de exacte spelersnaam zoals ingame."
                )
            return None

        player_id, exact_name = player_data

        exists = await self.db.fetchval("""
            SELECT 1
            FROM player_conquer_settings_v2
            WHERE guild_id = $1 AND channel_id = $2 AND world = $3 AND player_id = $4;
        """, guild_id, channel_id, world, player_id)

        if exists:
            await self.db.execute("""
                DELETE FROM player_conquer_settings_v2
                WHERE guild_id = $1 AND channel_id = $2 AND world = $3 AND player_id = $4;
            """, guild_id, channel_id, world, player_id)

            if channel:
                await channel.send(
                    f"Tracken van veroveringen voor speler `{exact_name}` op `{world}` uitgeschakeld."
                )

            any_left = await self.db.fetchval("SELECT 1 FROM player_conquer_settings_v2 LIMIT 1;")
            if not any_left and self.player_check_conquers.is_running():
                self.player_check_conquers.cancel()
        ...
        await self.db.execute("""
            INSERT INTO player_conquer_settings_v2 (guild_id, channel_id, world, player_id, starting_unix_timestamp)
            VALUES ($1, $2, $3, $4, $5)
            ON CONFLICT DO NOTHING;
        """, guild_id, channel_id, world, player_id, now_ts)

        await self.db.execute("""
            INSERT INTO player_conquer_world_state_v2 (world, last_since)
            VALUES ($1, $2)
        ...
        if channel:
            await channel.send(
                f"Tracken van veroveringen voor speler `{exact_name}` op `{world}` aangezet."
            )

        if self.bot.is_ready() and not self.player_check_conquers.is_running():
            self.player_check_conquers.start()
            logger.info("[PlayerConquers] Background player_check_conquers loop started via toggle.")
    ...
    async def _get_since_for_world(self, world: str) -> int:
        row = await self.db.fetchrow("""
            SELECT last_since
            FROM player_conquer_world_state_v2
            WHERE world = $1;
        """, world)
    ...
    async def player_check_conquers(self):
        if not self.bot.is_ready():
            return

        tracking_data = await self.db.fetch("""
            SELECT guild_id, channel_id, world, player_id
            FROM player_conquer_settings_v2;
        """)
            ...
            try:
                ...
                try:
                    async with self.session.get(url) as response:
                        if response.status != 200:
                            print(f"[PlayerConquers {world.upper()}] HTTP {response.status} bij ophalen conquers")
                            continue
                        raw_data = await response.text()
                except asyncio.TimeoutError:
                    print(f"[PlayerConquers {world.upper()}] timeout bij ophalen conquers")
                    continue
                except aiohttp.ClientError as e:
                    print(f"[PlayerConquers {world.upper()}] aiohttp fout: {e}")
                    continue

                conquers = [entry.split(",") for entry in re.split(r"\s+", raw_data.strip()) if entry]
                if not conquers:
                    print(f"[PlayerConquers {world.upper()}] - 0 new conquers found.")
                    continue
                ...
                for conquer in conquers:
                    ...
                    relevant = [
                        ...
                        if t["player_id"] in (new_owner_id, old_owner_id)
                    ]
                    if not relevant:
                        continue
                ...
                if stored_count == 0:
                    ...
                    print(f"[PlayerConquers {world.upper()}] - {stored_count} new conquers found.")

            except Exception as e:
                print(f"[PlayerConquers {world.upper()}] scan fout: {e}")

    @player_check_conquers.before_loop
    async def before_player_check_conquers(self):
        await self.bot.wait_until_ready()

    async def store_conquer(self, world, village_id, unix_timestamp, new_owner_id, old_owner_id):
        exists = await self.db.fetchval("""
            SELECT 1
            FROM player_conquer_data_v2
        ...
        await self.db.execute("""
            INSERT INTO player_conquer_data_v2 (
    ...
    async def process_conquer(
        ...
        old_owner_link = (
            f"[{old_owner_name}](https://{world}.tribalwars.nl/game.php?screen=info_player&id={old_owner_id})"
            if old_owner_id != 0 else old_owner_name
        )

        exists = await self.db.fetchval("""
            SELECT 1
            FROM player_conquer_messages_v2
        ...
        if new_owner_id == tracked_player_id and old_owner_id == 0:
            description = f"{new_owner_link} heeft een barbarendorp veroverd!"
            color = discord.Color.green()
        elif (
            new_owner_id == tracked_player_id
        ...
        elif new_owner_id == tracked_player_id and new_owner_id == old_owner_id:
            description = f"{new_owner_link} heeft zichzelf veroverd!"
            color = discord.Color.yellow()
        elif (
            old_owner_id == tracked_player_id
            and new_owner_tribe_id != old_owner_tribe_id
            ...
            color = discord.Color.red()
        elif new_owner_id == tracked_player_id and old_owner_tribe_id == 0:
        ...
        local_dt = datetime.fromtimestamp(unix_timestamp, tz=ZoneInfo("UTC")).astimezone(ZoneInfo("Europe/Amsterdam"))
        ...
        await self.db.execute("""
            INSERT INTO player_conquer_messages_v2 (
...
async def setup(bot: commands.Bot):
    await bot.add_cog(PlayerConquers(bot))
